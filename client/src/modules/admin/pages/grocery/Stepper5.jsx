...
const Stepper5 = ({ formData, setFormData, error, setError, handleNext, handleBack, onSuccess }) => {
  ...
  const submitToBackend = async () => {
    ...
    const apiUrl = "http://localhost:5000/api/v1/grocery-sellers/create-grocery-seller";
    ...
    try {
      ...
      const roleResponse = await axios.get("http://localhost:5000/api/v1/role/fetch-all-role", {
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
        },
      ...
      const userUpdateResponse = await axios.put(
        `http://localhost:5000/api/v1/users/update-users-by-id/${formData.user_id}`,
