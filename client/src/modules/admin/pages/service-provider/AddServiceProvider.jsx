...
const AddServiceProvider = ({ isOpen, onClose }) => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (step3.description.trim().split(/\s+/).length < 30) {
      ...
      if (!token) {
        token = localStorage.getItem("authToken") || localStorage.getItem("token"); // Try common localStorage keys
      }
          ...
          <div>
            ...
            {!searchAttempted || (!users && !loading && !error) ? (
              <div>
                ...
                />
                <Button onClick={handleSearch} className="bg-blue-500 text-white w-full">
