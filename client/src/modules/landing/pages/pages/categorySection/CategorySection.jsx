    ...
    <div className="p-4 space-y-8">
      {categories.map((category, index) => (
        <React.Fragment key={index}>
          {/* 👉 Category Block */}
          <div className="border rounded-lg shadow p-4 flex flex-col md:flex-row-reverse gap-4">
            {/* Right: Banner */}
            <div className="relative group w-full md:w-1/2 flex justify-center items-center h-auto rounded overflow-hidden">
              <div
                className="w-full h-full bg-cover bg-center transition-transform duration-500 group-hover:scale-110"
                style={{ backgroundImage: `url(${category.bannerImage})` }}
              ></div>
              <div className="absolute inset-0 bg-gradient-to-br from-black/60 via-black/40 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-500"></div>
              <div className="absolute inset-0 flex items-center justify-center">
                <h2 className="text-white text-xl md:text-2xl font-bold text-center bg-black/70 px-4 py-2 rounded transition-transform duration-500 group-hover:scale-105">
                  {category.name}
                </h2>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 w-full md:w-1/2">
              {category.subcategories.map((sub, subIdx) => (
                <div
                  key={subIdx}
                  className="border p-3 rounded-md flex gap-2 shadow-sm hover:shadow-md transition"
                >
                  <div>
                    <img
                      src={sub.image}
                  ...
                  <div>
                 <Link>
                 <h3 className="mt-2 font-semibold text-sm text-[#e03733]">{sub.name}</h3>
                 </Link>
          ...
          //  <CountryPage countries={countries}  />
           <CountryWrapper initialCountries={countries} />
          )}
        </React.Fragment>
      ))}
    </div>
