...
//   const { data, isLoading, isError } = useGetComplaintFormsQuery();
...
//           // Update uploadedImages per field
//           setUploadedImages((prev) => ({
//             ...prev,
//             [name]: [...(prev[name] || []), ...urls], // Add to the specific field's array
//           }));

//           // Update formData with the new uploaded images
//           setFormData((prev) => ({
//             ...prev,
//             [name]: [...(prev[name] || []), ...urls], // Add to the specific field's array
//           }));
...
//     if (!selectedOption) return setError("Please select complaint type");

//     if (
//       selectedOption.type === "type1" &&
//       (!formData.complaint_description || formData.complaint_description.length < 30)
//     ) {
//       return setError("Minimum 30 characters required in complaint description");
//     }
...
//       user_id: user?.user?._id || "",
//       details: {}, // We'll add dynamic details fields here
//     };

//     // Flatten details into the payload object as: details[field] = value
//     Object.entries(formData).forEach(([key, value]) => {
//       if (value !== null && value !== false && value !== "") {
//         // Handle array fields like attachments (send as an array of objects if needed)
//         if (Array.isArray(value)) {
//           payload.details[key] = value.map(v => JSON.stringify(v));
//         } else {
...
//     try {
//       if (isEditMode) {
//         await updateComplaintForm({ id: editId, body: payload }).unwrap();
...
//     // Find the corresponding option from complaintOptions based on the 'option' value
//     const selectedOption = complaintOptions.find((opt) => opt.value === edit.option);
  
//     // Patch the form data with the complaint details
//     setFormData({
//       ...formData,
//       option: edit.option || "",
//       type: edit.type || "",
...
//       court_order_attachment: edit.details?.court_order_attachment || [],
...
//       related_attachment: edit.details?.related_attachment || [],
//       attachment_1: edit.details?.attachment_1 || [],
//       copyright_title: edit.details?.copyright_title || "",
//     });
  
//     // Set the dropdown to the correct label
//     if (selectedOption) {
//       // Assuming the Select component allows setting the value programmatically
//       // You may need to use a ref or state to set the Select value
//       setSelectedOption(selectedOption.label);
//     }
//   };

//   const handleDeleteImage = async (fieldName, url) => {
//     console.log(fieldName, "delete1");
//     console.log(url, "delete2");
//     const complainName = url?.fileUrl.split("/");
//     const filename = url?.fileUrl;
//     const complaint_name = complainName[complainName?.length - 2]; // Get complaint name from the URL
...
//       // Remove from uploadedImages state for the specific field
//       setUploadedImages((prev) => ({
//         ...prev,
//         [fieldName]: prev[fieldName].filter((item) => item.fileUrl !== url.fileUrl),
//       }));

//       // Remove from formData state for the specific field
//       setFormData((prev) => ({
//         ...prev,
//         [fieldName]: prev[fieldName].filter((item) => item.fileUrl !== url.fileUrl),
//       }));
...
//                         name="agreement"
//                         type="checkbox"
//                         checked={formData.agreement} // Bind to formData
...
//         <ComplaintList onEdit={handleEdit} />
//       </div>
//     </div>
//   );
// };
